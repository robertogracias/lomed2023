...
class odoosv_user(models.Model):
    ...
    tipos_para_cierre=fields.Char(string='Typos a cerrar')
    fecha_cierre_1=fields.Date("Fecha de inicio para generar el cierre")
    fecha_cierre_2=fields.Date("Fecha de fin para generar el cierre")
    cuenta_utilidad=fields.Many2one(comodel_name='account.account',string="Cuenta de utilidad")
    partida_cierre=fields.Many2one(comodel_name='account.move',string='Partida de cierre generada')
    def crear_cierre_anual(self):
        for r in self:
            env['odoosv.cierre.detalle'].search([]).unlink()
            tipos=[]
            #for t in r.tipos_para_cierre:
            #    tipos.append(t.id)
            cuentas = env['account.account'].search([('account_type','in',tipos)])
            utilidad=0.0
            debetotal=0.0
            habertotal=0.0
            for c in cuentas:
                movimientos = env['account.move.line'].search([('account_id','=',c.id),('date','>=',r.fecha_cierre_1),('date','<=',r.fecha_cierre_1),('parent_state','=','posted')])
                debe = 0
                haber = 0
                for m in movimientos:
                    debe= debe+m.debit
                    haber = haber+m.credit
                if haber-debe != 0:
                    env['odoosv.cierre.detalle'].create({'cuenta_id':c.id,'saldo_deudor':debe,'saldo_acreedor':haber,'diferencia':round(haber-debe,2)})
            partida={}
            partida['name']='Cierre'
            partida['ref']='Cierre'
            journal_id=env['account.journal'].search([('type','=','general'),('company_id','=',r.id)],limit=1)
            partida['journal_id']=journal_id.id
            partida['company_id']=r.id
            partida['move_type']='entry'
            lines=[]
            listado = env['odoosv.cierre.detalle'].search([])
            for l in listado:
                linea={}
                linea['name']='liquidacion :'+l.cuenta_id.name
                linea['account_id']=l.cuenta_id.id
                utilidad=utilidad+l.diferencia
                if l.diferencia > 0:
                    linea['debit']=l.diferencia
                    linea['credit']=0
                    debetotal=debetotal+round(l.diferencia,2)
                else:
                    linea['debit']=0
                    linea['credit']=l.diferencia*-1
                    habertotal=habertotal+round((l.diferencia*-1),2)
                linea1=(0,0,linea)
                lines.append(linea1)
            #linea de utilidad
            linea={}
            linea['name']=r.name
            linea['account_id']=r.cuenta_utilidad.id
            if utilidad > 0:
                linea['debit']=0
                linea['credit']=utilidad
                habertotal=habertotal+round(utilidad,2)
            else:
                linea['debit']=utilidad*-1
                linea['credit']=0
                debetotal=debetotal+utilidad*-1
            linea1=(0,0,linea)
            lines.append(linea1)
            
            #linea de diferencia 
            diferencia=habertotal-debetotal
            linea={}
            linea['name']=r.name
            linea['account_id']=r.cuenta_utilidad.id
            if diferencia > 0:
                linea['debit']=0
                linea['credit']=diferencia
            else:
                linea['debit']=diferencia*-1
                linea['credit']=0
            linea1=(0,0,linea)
            #lines.append(linea1)
            partida['line_ids']=lines
            move=env['account.move'].create(partida)
            partida_cierre=move.id
...
class odoosv_move(models.Model):
    ...
    resolucion_fiscal = fields.Char(related="tipo_documento_id.resolucion_fiscal", string="N° Resolución Fical")
            ...
            if r.move_type in ('in_invoice','in_refund','out_invoice','out_refund'):
                if r.state!='draft':
                    if not r.tipo_documento_id:
                        if r.journal_id.tipo_documento_id:
                            r.tipo_documento_id = r.journal_id.tipo_documento_id.id
                        else:
...
class odoosv_documento(models.Model):
    ...
    resolucion_fiscal = fields.Char(string="N° Resolución fiscal")
...
class odoosv_sale_order(models.Model):
    ...
    def createxxx_invoices(self, grouped=False, final=False, date=None):
        """
...
class odoosv_pago_register(models.TransientModel):
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        payment_vals = super(odoosv_pago_register,self)._create_payment_vals_from_wizard(batch_result)
...
class cierre_detalle(models.Model):
    _name='odoosv.cierre.detalle'

    cuenta_id=fields.Many2one(comodel_name='account.account',string='Cuenta')
    saldo_deudor=fields.Float('Saldo Deudor')
    saldo_acreedor=fields.Float('Saldo Acreedor')

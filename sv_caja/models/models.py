...
class sucursales_sucursal(models.Model):
    ...
    documento_unico=fields.Boolean("Utiliza documento único")
    du_sequence_id=fields.Many2one('ir.sequence',string="Numeracion",ondelete="restrict")
...
class sucursales_account_move(models.Model):
    ...
    cierrea_id=fields.Many2one(comodel_name='odoosv.cierre', string="Cierre")
    serie=fields.Char('Serie')
    documento_unico=fields.Char("Documento unico")
...
class sucursales_cierre(models.Model):
    ...
    factura_anulada_ids=fields.One2many(comodel_name='account.move',inverse_name='cierrea_id',string="Facturas Anuladas")
    ...
    def calcular(self):
        for record in self:
            ...
            facturas=self.env['account.move'].search([('cierre_id','=',record.id)])
            for factura in facturas:
                factura.write({'cierre_id':False,'cierrea_id':False})
            pagos=self.env['account.payment'].search([('cierre_id','=',record.id)])
            for pago in pagos:
                pago.write({'cierre_id':False})
            cierres=self.env['odoosv.cierre.pago'].search([('cierre_id','=',record.id)])
            ...
            for remesa in remesas:
                ...
                dic['destino_id']=remesa.destination_journal_id.id
                dic['monto']=remesa.amount
                dic['cierre_id']=record.id
                self.env['odoosv.cierre.remesa'].create(dic)
...
class linea_de_apertura_caja(models.Model):
    ...
    billete_moneda_id = fields.Many2one(comodel_name="odoosv.cierre.bill", string="Monedas/Billetes")
...
class odoo_sv_bill(models.Model):
    _name='odoosv.cierre.bill'
    name=fields.Char("Billete Moneda")
...
class odoosv_pago_register(models.TransientModel):
    ...
    journal_id = fields.Many2one(
        ...
        domain="[('id', 'in', available_journal_ids),('caja_id','in',caja_id)]")

...
class Inventory(models.Model):
    ...
    inventory_type = fields.Selection(
        [('all', 'All Product'), ('categ', 'One Product Category'), ('one', 'Only One Product'),
         ('product', 'Select Product Manually'), ('lot', 'One Lot/Serial Number')], default='all',
        string='Inventory Type')
    categ_id = fields.Many2one('product.category')
    product_id = fields.Many2one('product.product')
    lot_id = fields.Many2one('stock.lot')

    @api.onchange('inventory_type', 'categ_id', 'lot_id', 'product_id')
    def onchange_inventory_type(self):
        for rec in self:
            ...
            if rec.inventory_type == 'all':
                ...
                print(rec.categ_id.name)
                product_ids = self.env['product.product'].search([('categ_id', '=', rec.categ_id.id)])
                print(product_ids, 'ooo')
                rec.product_ids = [(6, 0, product_ids.ids)]
    ...
    def action_open_inventory_lines(self):
        ...
        if self.product_ids:
            ...
            action['view_id'] = self.env.ref('inventory_adjustment_screen_app.stock_inventory_line_tree_no_product_create').id
            if len(self.product_ids) == 1:
                context['default_product_id'] = self.product_ids[0].id
        else:
            # no product_ids => we're allowed to create new products in tree
            action['view_id'] = self.env.ref('inventory_adjustment_screen_app.stock_inventory_line_tree').id
    ...
    def action_print(self):
        return self.env.ref('inventory_adjustment_screen_app.action_report_stock_inventory').report_action(self)
...
class InventoryLine(models.Model):
    ...
    def _search_difference_qty(self, operator, value):
        ...
        line_ids = lines.filtered(lambda line: float_is_zero(line.difference_qty,
                                                             precision_rounding=line.product_id.uom_id.rounding) == result).ids
